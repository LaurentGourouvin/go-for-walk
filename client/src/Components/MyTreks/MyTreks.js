...
function MyTreks({ token }) {
  ...
  if (!isLogged) {
    return (
      <p className="msg-error p-2 bg-amber-50 rounded-md shadow-md">
        <img src={ImageWarning} alt="logo de warning" />
        Vous devez être connecté pour consultez vos randonnées
      </p>
    );
  }
  ...
  return (
    <>
      ...
      <div className="MyTreks">
        {treksByUserId.map((trek) => (
          <Trek key={trek.id} data={trek} token={token} setTreksByUserId={setTreksByUserId} />
        ))}
      </div>
    </>
  );
